...
if (count($_FILES)) {//если файл пришел
  ...
  if (in_array($file['type'], $validFileType, true) && $file['size'] <= $validSizeImage) {
    ...
    while (in_array($newNameImage, $arrFiles, true)) {
      ...
      if (!$matches) {
        ...
        if (!preg_match($regPattern, $file['name'], $matches) || count($matches) !== 3) {
    ...
    if (move_uploaded_file($file['tmp_name'], IMG_MAX_DIR . $newNameImage)) {
      ...
      if (!img_resize(IMG_MAX_DIR . $newNameImage, IMG_MIN_DIR . $newNameImage, $imageWidth, $imageHeight)) {
