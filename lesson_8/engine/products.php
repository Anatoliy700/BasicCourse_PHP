  ...
  switch ($type) {
    ...
    case 'add':
      ...
      if ($product_id && $product_amount) {
        ...
        if (isset($_SESSION['cart']['product'])) {
          foreach ($_SESSION['cart']['product'] as $key => $item) {
        ...
        if ($arrKey !== false) {
          ...
          if ($update) {
            $_SESSION['cart']['product'][$arrKey]['amount'] = $product_amount;
          } else {
            $_SESSION['cart']['product'][$arrKey]['amount'] += $product_amount;
          }
          //если добавляемые товар не присутсвует в карзине
        } else {
          $_SESSION['cart']['product'][] = ['id' => $product_id, 'amount' => $product_amount];
        }
        return true;
      }
      break;
    ...
    case 'remove':
      ...
      if ($product_id) {
        foreach ($_SESSION['cart']['product'] as $key => $item) {
          if ($item['id'] === $product_id) {
            array_splice($_SESSION['cart']['product'], $key, 1);
            break;
          }
        }
        return true;
...
function getProductOfCart($arrProductCart) {
  ...
  if ($arrId) {
    ...
    $out['totalAmountProducts'] = $totalAmountProducts;
